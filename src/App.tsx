...
export default function App() {
  ...
  const showMenu = (event: React.MouseEvent, ship: ShipState) => {
    ...
    if (y + estimatedHeight > screenH - 60) {
      y = Math.max(50, screenH - 60 - estimatedHeight);
    }
    ...
    <div style={{ margin: 0, background: '#080604', overflow: 'hidden', width: '100vw', height: '100dvh', position: 'relative' }}>
      <style>{`
        :root {
            ...
            --building-img-size: 96px;
            --building-hotspot-width: 106px;
        ...
        @media (max-width: 768px) {
            :root {
                --ship-render-width: 90px;
                --building-img-size: 78px;
                --building-hotspot-width: 88px;
        ...
        .top-bar { position: fixed; top: 6px; width: 95%; left: 2.5%; display: flex; justify-content: space-between; gap: 6px; z-index: 15; }
        .resource-box { background: rgba(40, 30, 20, 0.94); border: 1.5px solid #ca8a04; padding: 3px 6px; border-radius: 7px; display: flex; flex-direction: column; align-items: center; min-width: 72px; color: #fff; cursor: pointer; transition: all 0.2s ease; box-shadow: 0 3px 10px rgba(0,0,0,0.7); flex: 1; }
        .resource-box:hover { background: rgba(55, 40, 30, 0.98); transform: scale(1.03); }
        .res-icon { width: 26px; height: 26px; }
        .res-label { font-size: 10.5px; border-top: 1px solid #5d4037; padding-top: 1px; margin-top: 1px; width: 100%; text-align: center; font-weight: 800; color: #fef08a; text-shadow: 0 1px 2px rgba(0,0,0,0.9); }
